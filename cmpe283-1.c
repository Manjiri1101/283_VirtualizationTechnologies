...
/* From SDM Table Table 24-11. */

struct capability_info exit_control[14] =
{
...
/* From SDM Table 24-13. */

struct capability_info entry_control[12]=
{
...
detect_vmx_features(void)
{
	...
	pr_info(" Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exit_control, 14, lo, hi);
	...
	pr_info(" Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entry_control, 12, lo, hi);
