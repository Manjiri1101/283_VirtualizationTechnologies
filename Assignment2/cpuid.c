...
atomic_t exitnumbers = ATOMIC_INIT(0);
EXPORT_SYMBOL(exitnumbers);
...
/*
* Added below code for Assignment 2 create  new CPUID leaf 0x4FFFFFFF)
*
*/
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if(eax == 0x4FFFFFFF){
		printk("CPUID(0x4FFFFFFF), Number of exit: %d\n", arch_atomic_read(&exitnumbers));
		eax = arch_atomic_read(&exitnumbers);
		
		cpu_cycles_copy = arch_atomic64_read(&cpu_cycles);
		ebx = (cpu_cycles_copy >> 32);;	
		ecx = (cpu_cycles_copy & 0xffffffff); 
		edx = 0;
	}
	else{
